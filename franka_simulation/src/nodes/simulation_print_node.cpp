// =============================================================================
// Name        : simulation_print_node.cpp
// Author      : Rita Gama
// Description :
// =============================================================================
...
int main(int argc, char **argv){
    ...
    ros::init(argc, argv, "simulation_print_node");
    ...
    // ---------------------------------------------------------------------------
    Eigen::Vector3d pf2;
    pf2 << 0.565211, -0.0351449, 0.474653; //finish position

    Eigen::Quaterniond Qpoints2, of2;
    Qpoints2.coeffs() << 0.999828, -0.00776464, -0.000869379, 0.0168194; // finish orientation
    Qpoints2.normalize();
    of2.coeffs() << Qpoints2.vec()[0], Qpoints2.vec()[1], Qpoints2.vec()[2], Qpoints2.w();
        ...
        switch (flag_pattern) {
            ...
            case PRINT:
                ...
                if (count < n_pattern-1){
                    ...
                    //Set new orientation
                    oi.coeffs() << pose_i[3], pose_i[4], pose_i[5], pose_i[6];
                    Qmold_aux.coeffs() << orientation(0, count), orientation(1, count), orientation(2, count), orientation(3, count);
                    Qmold_aux.normalize();
                    Rmould = Qmold_aux;
                    Rpoints = Rmould*Rpanda;
                    Qpoints = Rpoints;
                    ...
                    std::cout << CLEANWINDOW << "FINISHED PRINT. GOING HOME POSITION!" << std::endl;
            ...
            case HOME:
                ...
                // --> MOVE TO A MOLD POINT <--
                if( (t >= ti) && (t <= tf) ){
                    pd = panda.polynomial3Trajectory(pi, pf2, ti, tf, t);
                    if ( t1 <= 1.0 ){
                        od = oi.slerp(t1, of2);
                        od.normalize();
                    }
                    ...
                    std::cout << CLEANWINDOW << "PRINTING TIME WAS: " << t_aux << "s "<< std::endl;
